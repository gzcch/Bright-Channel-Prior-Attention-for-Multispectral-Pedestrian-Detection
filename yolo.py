...
class YOLO(object):
    ...
    #   检测图片
    # ---------------------------------------------------#
    def detect_image(self, image_rgb, image_gray, crop=False, count=False):
        # ---------------------------------------------------#
        #   计算输入图片的高和宽
        # ---------------------------------------------------#
        ...
        image_gray = cvtColor(image_gray)
        # ---------------------------------------------------------#
        #   给图像增加灰条，实现不失真的resize
        #   也可以直接resize进行识别
        # ---------------------------------------------------------#
        image_rgb_data = resize_image(image_rgb, (self.input_shape[1], self.input_shape[0]), self.letterbox_image)
        image_gray_data = resize_image(image_gray, (self.input_shape[1], self.input_shape[0]), self.letterbox_image)
        image = np.concatenate(
            (np.array(image_rgb_data, dtype='float32'), np.array(image_gray_data, dtype='float32')[:, :, 0][..., None]),
            axis=2)
        # ---------------------------------------------------------#
        #   添加上batch_size维度
        # ---------------------------------------------------------#
        ...
        thickness = int(max((h + w) // np.mean(self.input_shape), 1))
        # ---------------------------------------------------------#
        #   计数
        # ---------------------------------------------------------#
        if count:
        ...
        if crop:
            for i, c in list(enumerate(top_label)):
                ...
                bottom = min(w, np.floor(bottom).astype('int32'))
                right = min(h, np.floor(right).astype('int32'))
